...
public class RouteController {
    ...
    @POST
    @Path("/{id}/start")
    @RolesAllowed({"COURIER"}) // Tylko kurier może zacząć jazdę
    public Response startRoute(@PathParam("id") UUID routeId) {
        try {
            String userId = jwt.getSubject();
            routeService.startRoute(routeId, userId);
            return Response.ok("{\"message\": \"Trasa rozpoczęta pomyślnie\"}").build();
        } catch (Exception e) {
    ...
    public Response completeStop(@PathParam("id") UUID stopId) {
        try {
            ...
            return Response.ok("{\"message\": \"Zaktualizowano status przystanku\"}").build();
        } catch (Exception e) {
            return Response.status(Response.Status.BAD_REQUEST).entity("{\"error\": \"" + e.getMessage() + "\"}").build();
        }
    }
    ...
    public Response finishRoute(@PathParam("id") UUID routeId) {
        try {
            ...
            return Response.ok("{\"message\": \"Zmiana zakończona. Paczki oddane do sortowni.\"}").build();
        } catch (Exception e) {
            return Response.status(Response.Status.BAD_REQUEST).entity("{\"error\": \"" + e.getMessage() + "\"}").build();
        }
    }
